...
class ApproximatedEMDLoss(nn.Module):
    ...
    def forward(self, x, y):
        return ApproximatedEMD.apply(x, y, self.epsilon, self.max_iter)


class ApproximatedEMD(autograd.Function):
    @staticmethod
    def forward(ctx, x, y, epsilon, max_iter):
        batch_size, num_points, _ = x.size()
        cost_matrix = torch.cdist(x, y, p=2)  # Pairwise Euclidean distance
        ctx.save_for_backward(x, y, cost_matrix)
        ctx.epsilon = epsilon
        ctx.max_iter = max_iter

        # Uniform distributions for source and target
        r = torch.ones(batch_size, num_points, device=x.device) / num_points
        c = torch.ones(batch_size, num_points, device=y.device) / num_points
    ...
    def backward(ctx, grad_output):
        x, y, cost_matrix = ctx.saved_tensors
        epsilon = ctx.epsilon
        max_iter = ctx.max_iter
        batch_size, num_points, dim = x.size()

        # Recompute the kernel matrix for gradients
        K = torch.exp(-cost_matrix / epsilon)
        ...
        for i in range(batch_size):
            grad_matrix = (1 / epsilon) * (x[i].unsqueeze(1) - y[i].unsqueeze(0))
            grad_x[i] = torch.sum(grad_matrix * K[i], dim=1)
            grad_y[i] = -torch.sum(grad_matrix * K[i], dim=0)

        return grad_x * grad_output, grad_y * grad_output, None, None
